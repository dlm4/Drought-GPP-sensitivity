...
flx_site_info <- read.csv("C:/Users/David Miller/Documents/dlm_files/keenan_postdoc/fluxnet2015/fluxnet2015_site_locations_plaintext.csv")
...
setwd("C:/Users/David Miller/Documents/dlm_files/keenan_postdoc/trendy/gpp_precip_slope_v4")
...
for (trendyfile in file_list){
  ...
  #setwd("C:/Users/David Miller/Documents/dlm_files/keenan_postdoc/trendy/gpp_precip_slope_v4_southhem")
  ...
  season_name <- splitname[length(splitname)-1]
  ...
  flx_site_info_sub$Climate2 <- ec_gpp_sens_sub$Climate2
  ...
  # save plot
  ggsave(paste("../results/v4_ec_site_locations/", splitname[1], "_gpp_precip_ec_sites_", season_name, ".png", sep = ""), width = 5, height = 4, units = "in")
  ...
  # save plot
  ggsave(paste("../results/v4_ec_site_locations/", splitname[1], "_gpp_precip_ec_sites_", season_name, "_log10aridity.png", sep = ""), width = 5, height = 4, units = "in")
  
  # write out data frame to csv
  write.csv(flx_site_output, paste("../results/v4_ec_site_locations/", splitname[1], "_gpp_precip_ec_sites_data_", season_name, ".csv", sep = ""), row.names = F)
...
write.csv(flx_site_output_all, paste("../results/v4_ec_site_locations/allmodels_gpp_precip_ec_sites_data.csv", sep = ""), row.names = F)

#####
# model regression using all models (excluding SDGVM)

setwd("../results/v4_ec_site_locations/")
...
flx_site_output_all_noSDGVM_mean <- aggregate(flx_site_output_all_noSDGVM, 
                                              by = list(flx_site_output_all_noSDGVM$SITE_ID, flx_site_output_all_noSDGVM$Season,
                                                        flx_site_output_all_noSDGVM$Climate2), 
                                              FUN = mean, na.rm = T)

flx_site_output_all_noSDGVM_mean$Climate2 <- flx_site_output_all_noSDGVM_mean$Group.3
...
# log10 scaled version
ggplot(data = flx_site_output_all_noSDGVM_mean, aes(x = log10(Aridity), y = Precip_Slope)) +
  geom_hline(yintercept = 0) +
  geom_text(aes(label = SITE_ID, color = Climate2), size = 2, fontface = "bold") +
  #geom_point(aes(color = Climate2), size = 1) +
  geom_smooth(method = "lm", color = "black") + # linear
  facet_wrap(~Season, ncol = 3) +
  scale_y_continuous(breaks = seq(-3, 10)) + # y ticks
  stat_poly_eq(formula = my.formula, 
               aes(x = log10(Aridity), y = Precip_Slope, label = paste(..eq.label.., sep = "")),
  ...
  labs(x = "Log10 Aridity (P/PET)", y = "Slope GPP ~ Precip [g C m-2 / mm]",
       title = "Model mean: GPP ~ Precip") + 
  theme_bw()
ggsave("all_models_mean_gpp_precip_log10aridity.png", width = 12, height = 4, units = "in")
