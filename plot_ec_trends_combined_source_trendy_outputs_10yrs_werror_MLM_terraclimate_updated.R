...
t.test(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "wet")]) # flx wet sites precip not significantly different from zero (p = 0.2175)
...
p_spring_slope_precip_igbp <- ggplot(data = flx_slopes_precip, aes(x = TC_aridity, y = Slope)) +
  ...
  annotate("text", 0.15, y = -2, label = "Dry", color = dry_hex) + annotate("text", 2.9, y = -2, label = "Wet", color = wet_hex) + #"Wet \u2192"
...
ggsave("compare_mlm_ec_trendy_spring_terraclimate_updated_log10x.png", p_compare_mlm, width = 11, height = 11, units = "in")
