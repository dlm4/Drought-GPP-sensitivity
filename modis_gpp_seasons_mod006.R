...
gpp <- read.csv("modis_gpp_ec_sites_request4/EC-sites-GPP-4-MOD17A2HGF-006-results.csv")
...
for (id_num in 1:length(site_id_list)){
  ...
  # ggplot(data = gpp_sub) +
  #   geom_point(aes(x = Date, y = MOD17A2HGF_006_Gpp_500m))
  ...
  for (y in year_list){
    #y <- 2002
    gpp_sub2 <- subset(gpp_sub, year(Date) == y, c(Date, MOD17A2HGF_006_Gpp_500m))
    ...
    for (i in 1:nrow(gpp_sub2)){
      daily_gpp <- gpp_sub2$MOD17A2HGF_006_Gpp_500m[i]/8 # kg C / m2 / day, scale factor is already applied
...
write.csv(gpp_out, file = "../ec_sites_seasons_modis_gpp_MOD17A2HGF_006_Gpp_500m.csv", row.names = F)
