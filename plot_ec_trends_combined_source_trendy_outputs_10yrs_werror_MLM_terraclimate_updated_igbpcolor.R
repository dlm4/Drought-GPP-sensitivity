# Figure 3/4!
# updated: MLM is now a supplemental figure
...
flx_slope_merged <- merge(slope_data, flx_info_merged, by.y = "SITE_ID")

flx_slope_merged_sub <- subset(flx_slope_merged, subset = Season == "Spring" & Npts >= 10)

# New output directory for terraclimate version
...
trendy_met_sub <- subset(trendy_slope_merged, select = c(TC_aridity, Precip_Slope, Precip_Slope_SE, Ta_Slope, Ta_Slope_SE, PAR_Slope, PAR_Slope_SE))
trendy_met_mean <- aggregate(trendy_met_sub, by = list(trendy_slope_merged$SITE_ID), FUN = mean)
colnames(trendy_met_mean)[1] <- "SITE_ID"

trendy_met_mean_slope <- subset(trendy_met_mean, select = c(SITE_ID, TC_aridity, Precip_Slope, Ta_Slope, PAR_Slope))
trendy_met_mean_slope_melt <- melt(trendy_met_mean_slope, id.vars = c("SITE_ID", "TC_aridity"))
colnames(trendy_met_mean_slope_melt)[3:4] <- c("Met_Var", "Slope")

trendy_met_mean_slope_se <- subset(trendy_met_mean, select = c(SITE_ID, TC_aridity, Precip_Slope_SE, Ta_Slope_SE, PAR_Slope_SE))
trendy_met_mean_slope_se_melt <- melt(trendy_met_mean_slope_se, id.vars = c("SITE_ID", "TC_aridity"))
...
# ggplot(trendy_met_mean_slope_and_se, aes(x = TC_aridity, y = Slope)) +
...
flx_slopes_allvars <- subset(flx_slope_merged_sub, select = c(SITE_ID, Met_Var, Season, Npts, MLM_Slope, Slope_SE, LOCATION_LAT, LOCATION_LONG, LOCATION_ELEV, IGBP, MAT, MAP, Climate, TC_aridity, CRU_P_Annual_Mean_Total_mm, CRU_PET_Annual_Mean_Total_mm))
flx_slopes_allvars$Source <- "EC"
colnames(flx_slopes_allvars)[which(colnames(flx_slopes_allvars) == "MLM_Slope")] <- "Slope" # relabel
flx_slopes_allvars$aridity_bins <- cut(flx_slopes_allvars$TC_aridity, breaks = c(0, 0.65, 10))
flx_slopes_allvars$aridity_labels <- "Energy-limited" #"wet"
flx_slopes_allvars$aridity_labels[which(flx_slopes_allvars$aridity_bins == "(0,0.65]")] <- "Water-limited" # "dry"
flx_slopes_allvars$aridity_labels <- factor(flx_slopes_allvars$aridity_labels, c("Water-limited", "Energy-limited"))
...
p_flx_trendy_precip_mean_color <- ggplot(flx_trendy_precip) +
  ...
  theme(legend.position = c(0.5, 0.8), # c(0.7, 0.8),
...
#t.test(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "Energy-limited")]) # flx wet sites precip not significantly different from zero (p = 0.2175)
#t.test(trendy_slopes_precip$Slope[which(trendy_slopes_precip$aridity_labels == "Energy-limited")]) # trendy wet sites precip, significantly positive (p << 0.0001)
...
# Precip
# flx
p_spring_slope_precip_igbp <- ggplot(data = flx_slopes_precip, aes(x = TC_aridity, y = Slope)) +
  ...
  guides(fill = "none") +
  annotate("text", x = 0.15, y = 6, label = "EC") +
  annotate("text", 0.2, y = -2, label = "Water-limited", color = dry_hex) + annotate("text", 1.8, y = -2, label = "Energy-limited", color = wet_hex) + #"Wet \u2192"
...
# trendy
p_trendy_spring_slope_precip_igbp <- ggplot(data = trendy_slopes_precip, aes(x = TC_aridity, y = Slope)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0.65, linetype = "dotted") +
  #geom_smooth(data = trendy_slope_merged_sub_spring, aes(x = TC_aridity, y = Precip_Slope, group = ModelName), 
  ...
  annotate("text", x = 0.2, y = 6, label = "TRENDY") +
  annotate("text", 0.2, y = -2, label = "Water-limited", color = dry_hex) + annotate("text", 1.8, y = -2, label = "Energy-limited", color = wet_hex) + #"Wet \u2192"
  scale_fill_manual(breaks = igbp_names, values = igbp_colors) +
  scale_color_manual(breaks = igbp_names, values = igbp_colors, guide = "none") +
  theme(legend.position = c(0.6, 0.625), 
    legend.key = element_blank(), legend.background = element_rect(fill = "white"),
    panel.background = element_blank(), panel.border = element_rect(color = "black", fill = NA),
    axis.title.x=element_blank(), axis.title.y=element_blank())
...
# PAR
# flx
p_spring_slope_par_igbp <- ggplot(data = flx_slopes_par, aes(x = TC_aridity, y = Slope)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0.65, linetype = "dotted") +
  geom_smooth(method = "lm", color = "black", se = FALSE) + #, formula = log.formula) +
  geom_errorbar(aes(ymin = Slope - Slope_SE, ymax = Slope + Slope_SE, color = IGBP)) +
  ...
  scale_x_continuous(trans = "log10", breaks = c(0.3, 0.65, 1, 3),
                     labels = as.character(c(0.3, 0.65, 1, 3))) + 
  labs(x = "Wetness Index (P/PET)", y = "GPP ~ PAR slope [g C / MJ]", fill = "IGBP") +
  scale_color_manual(breaks = igbp_names, values = igbp_colors, guide = "none") +
  scale_fill_manual(breaks = igbp_names, values = igbp_colors) +
  guides(fill = "none") + 
...
# trendy
p_trendy_spring_slope_par_igbp <- ggplot(data = trendy_slopes_par, aes(x = TC_aridity, y = Slope)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0.65, linetype = "dotted") +
  #geom_smooth(data = trendy_slope_merged_sub_spring, aes(x = TC_aridity, y = PAR_Slope, group = ModelName), 
  #            method = "lm", se = FALSE, formula = log.formula, size = 0.5, color = "gray80") + # add this for individual models
  geom_smooth(method = "lm", color = "black", se = FALSE) + #, formula = log.formula) +
  geom_errorbar(aes(ymin = Slope - Slope_SE, ymax = Slope + Slope_SE, color = IGBP)) +
  geom_point(aes(fill = IGBP), color = "black", size = 2, shape = 21) +
...
# Ta
# flx
p_spring_slope_ta_igbp <- ggplot(data = flx_slopes_ta, aes(x = TC_aridity, y = Slope)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0.65, linetype = "dotted") +
  geom_smooth(method = "lm", color = "black", se = FALSE) + #, formula = log.formula) +
  geom_errorbar(aes(ymin = Slope - Slope_SE, ymax = Slope + Slope_SE, color = IGBP)) + 
...
# trendy
p_trendy_spring_slope_ta_igbp <- ggplot(data = trendy_slopes_ta, aes(x = TC_aridity, y = Slope)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0.65, linetype = "dotted") +
  #geom_smooth(data = trendy_slope_merged_sub_spring, aes(x = TC_aridity, y = Ta_Slope, group = ModelName), 
  #            method = "lm", se = FALSE, formula = log.formula, size = 0.5, color = "gray80") + # add this for individual models
  geom_smooth(method = "lm", color = "black", se = FALSE) + #, formula = log.formula) +
  geom_errorbar(aes(ymin = Slope - Slope_SE, ymax = Slope + Slope_SE, color = IGBP)) + 
  geom_point(aes(fill = IGBP), color = "black", size = 2, shape = 21) +
...
ggsave("compare_mlm_ec_trendy_spring_terraclimate_updated_log10x_v2.png", p_compare_mlm, width = 11, height = 11, units = "in")



#####
# t tests and comparisons
t.test(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "wet")]) # ec wet slopes mean sig dif from zero? no, p = 0.2175
t.test(trendy_slopes_precip$Slope[which(trendy_slopes_precip$aridity_labels == "wet")]) # ec wet slopes mean sig dif from zero? yes, p = 2.382e-14
t.test(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "wet")] - trendy_slopes_precip$Slope[which(trendy_slopes_precip$aridity_labels == "wet")]) # ec - trendy wet slopes mean sig dif from zero? yes, p = 3.694e-06

# Number of wet sites with negative partial sensitivities to precip
length(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "wet")]) # 46
length(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "wet" & flx_slopes_precip$Slope < 0)]) # 28

# Number of dry sites with positive partial sensitivities to precip
length(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "dry")]) # 15
length(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "dry" & flx_slopes_precip$Slope > 0)]) # 9

# Number of wet sites with positive partial sensitivities to ta
length(flx_slopes_ta$Slope[which(flx_slopes_ta$aridity_labels == "wet")]) # 46
length(flx_slopes_ta$Slope[which(flx_slopes_ta$aridity_labels == "wet" & flx_slopes_ta$Slope > 0)]) # 43

# Number of dry sites with positive partial sensitivities to ta
length(flx_slopes_ta$Slope[which(flx_slopes_ta$aridity_labels == "dry")]) # 15
length(flx_slopes_ta$Slope[which(flx_slopes_ta$aridity_labels == "dry" & flx_slopes_ta$Slope < 0)]) # 9

#####
# other t-test checks

# dry, precip
t.test(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "dry")]) # ec dry slopes mean sig dif from zero? no, p = 0.05428
t.test(trendy_slopes_precip$Slope[which(trendy_slopes_precip$aridity_labels == "dry")]) # ec dry slopes mean sig dif from zero? yes, p = 9.646e-07
t.test(flx_slopes_precip$Slope[which(flx_slopes_precip$aridity_labels == "dry")] - trendy_slopes_precip$Slope[which(trendy_slopes_precip$aridity_labels == "dry")]) # ec - trendy wet slopes mean sig dif from zero? no, p = 0.5464

# wet, ta
t.test(flx_slopes_ta$Slope[which(flx_slopes_ta$aridity_labels == "wet")]) # ec dry slopes mean sig dif from zero? yes, p = 7.798e-07
t.test(trendy_slopes_ta$Slope[which(trendy_slopes_ta$aridity_labels == "wet")]) # ec dry slopes mean sig dif from zero? no, p < 2.2e-16
t.test(flx_slopes_ta$Slope[which(flx_slopes_ta$aridity_labels == "wet")] - trendy_slopes_ta$Slope[which(trendy_slopes_ta$aridity_labels == "wet")]) # ec - trendy wet slopes mean sig dif from zero? yes, p = 0.0108

# dry, ta
t.test(flx_slopes_ta$Slope[which(flx_slopes_ta$aridity_labels == "dry")]) # ec dry slopes mean sig dif from zero? no, p = 0.3208
t.test(trendy_slopes_ta$Slope[which(trendy_slopes_ta$aridity_labels == "dry")]) # ec dry slopes mean sig dif from zero? no, p = 0.6976
t.test(flx_slopes_ta$Slope[which(flx_slopes_ta$aridity_labels == "dry")] - trendy_slopes_ta$Slope[which(trendy_slopes_ta$aridity_labels == "dry")]) # ec - trendy wet slopes mean sig dif from zero? no, p = 0.3284
