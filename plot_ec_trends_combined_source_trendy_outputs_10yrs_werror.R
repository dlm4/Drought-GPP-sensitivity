...
trendy_sub <- subset(trendy_slope_merged_spring, select = c(CRU_Aridity_P_PET, Precip_Slope, Precip_Slope_SE))
...
# subset, no LPX
trendy_sub_noLPX <- subset(trendy_slope_merged_spring, subset = ModelName != "LPX", select = c(CRU_Aridity_P_PET, Precip_Slope, Precip_Slope_SE))
...
p_trendy_plot_models_color <- ggplot(trendy_plot_models, aes(x = ModelName, y = Precip_Slope, fill = aridity_labels)) +
  ...
  theme(legend.position = c(0.9, 0.8), 
        ...
        axis.ticks.y=element_blank())
#p_trendy_plot_models_wet
trendy_precip_boxplots <- ggarrange(p_flx_trendy_mean_color, p_trendy_plot_models_color, nrow = 1, align = "h", widths = c(1,3), labels = c("c", ""))
ggsave("figures/ec_trendy_boxplot_compare_10yrs_v2.png", trendy_precip_boxplots, height = 90, width = 189, units = "mm")
...
#
p_spring_slope_allmodels_igbp <- ggplot(data = trendy_plot_data, aes(x = CRU_Aridity_P_PET, y = Slope)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = 0.65, linetype = "dotted") +
  geom_smooth(data = trendy_plot_models, aes(x = CRU_Aridity_P_PET, y = Precip_Slope, group = ModelName), 
              method = "lm", se = FALSE, formula = log.formula, size = 0.5, color = "gray80") + # add this for individual models
