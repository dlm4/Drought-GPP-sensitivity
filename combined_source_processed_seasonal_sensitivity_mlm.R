...
options(scipen = 999) # avoid displaying values as scientific notation, large digit penalty

# NEW VERSION: March 16, 2022
# Set up for multilinear regression using precip, PAR, and Ta

# Load data file with all seasons
# setwd("C:/Users/David Miller/Documents/dlm_files/keenan_postdoc/fluxnet2015/processed_data/")
# df_all <- read.csv("fluxnet2015_info_swc1.csv")
...
#####
# Energy conversions below are all bonus
...
# Multilinear regression for each site with GPP ~ f(precip, PAR, Ta)
num_rows <- 0 # not sure how many rows yet, would be: number of sites * number of met vars * number of seasons?
df_sens_mlm_colnames <- c("SITE_ID", "Met_Var", "Season", "Npts", "MLM_Slope", "MLM_pvalue", "VIF", "Partial_PearsonR", "Partial_SpearmanR", "Partial_KendallR", "Slope_SE")
df_sens_mlm <- data.frame(matrix(data = NA, nrow = num_rows, ncol = length(df_sens_mlm_colnames)))
colnames(df_sens_mlm) <- df_sens_mlm_colnames

#site_id <- "CH-Aws"
#seas <- "Spring"

#df_test <- subset(df_all, SITE_ID == "CH-Aws")

for (site_id in unique(df_all$SITE_ID)){
  for (seas in unique(df_all$season)){
    
    print(site_id)
    
    # set up for one site, one season, for 3 variables, will loop
    df_sens_mlm_sub <- data.frame(matrix(data = NA, nrow = 3, ncol = length(df_sens_mlm_colnames)))
    colnames(df_sens_mlm_sub) <- df_sens_mlm_colnames
    
    df_sens_mlm_sub$SITE_ID <- site_id
    df_sens_mlm_sub$Season <- seas
    
    df_sens_mlm_sub$Met_Var <- c("Precip", "PAR", "Ta")
    
    df_all_sub <- subset(df_all, SITE_ID == site_id & season == seas, select = c(GPP_detrend, P_F_sum, PAR_F_sum, TA_F_mean))
    df_all_sub <- na.omit(df_all_sub) # remove rows that have NA
    
    if (nrow(df_all_sub) >= 5){
      
      # GPP_detrend is the column for GPP to use, since it might be detrended if necessary per season
      gpp <- df_all_sub$GPP_detrend
      precip <- df_all_sub$P_F_sum
      par <- df_all_sub$PAR_F_sum
      ta <- df_all_sub$TA_F_mean
      
      # version that does rescaling or normalizing would do the normalization step here, then do the same thing afterwards
      
      mlm_fit <- lm(gpp ~ precip + par + ta)
      mlm_fit_summary <- summary(mlm_fit)
      
      df_sens_mlm_sub$MLM_Slope <- mlm_fit_summary$coefficients[2:4,1] # get slope
      df_sens_mlm_sub$MLM_pvalue <- mlm_fit_summary$coefficients[2:4,4] # get p value
      
      # (added Mar 31, 2022)
      # Slope standard error
      df_sens_mlm_sub$Slope_SE <- mlm_fit_summary$coefficients[2:4,2]
      
      df_sens_mlm_sub$Npts <- nrow(df_all_sub)
      
      # Variance inflation factor
      df_sens_mlm_sub$VIF <- vif(mlm_fit)
      
      # pearson correlation
      input_df <- cbind.data.frame(gpp, precip, par, ta)
      pcor_pearson <- pcor(input_df, method = "pearson")
      # use output of first row or first column of $estimate,
      # this is the partial correlation of each variable with gpp given the other two
      # careful with this indexing!
      df_sens_mlm_sub$Partial_PearsonR <- pcor_pearson$estimate[2:4,1]
      
      # only do these if at least 6, otherwise NA
      if (nrow(df_all_sub) >= 6){
        # these cannot be calculated if there are only 5 data points for 3 variables
        # spearman correlation
        pcor_spearman <- pcor(input_df, method = "spearman")
        df_sens_mlm_sub$Partial_SpearmanR <- pcor_spearman$estimate[2:4,1]
        
        # kendall correlation (non-parametric, similar to spearman)
        pcor_kendall <- pcor(input_df, method = "kendall")
        df_sens_mlm_sub$Partial_KendallR <- pcor_kendall$estimate[2:4,1]
      }
      
      # then bind together, row by row
      df_sens_mlm <- rbind.data.frame(df_sens_mlm, df_sens_mlm_sub)
    }
  }
}

# could add in the univariate stuff above and do it all in one shot.
# then sub in trendy version of variables, and also the rescaled variants

# remove sites that are missing a season
sites_spring <- unique(df_sens_mlm$SITE_ID[which(df_sens_mlm$Season == "Spring")])
sites_summer <- unique(df_sens_mlm$SITE_ID[which(df_sens_mlm$Season == "Summer")])
sites_fall <- unique(df_sens_mlm$SITE_ID[which(df_sens_mlm$Season == "Fall")])
common_site_list <- Reduce(intersect, list(sites_spring, sites_summer, sites_fall)) # multiple intersect

df_sens_mlm_output <- subset(df_sens_mlm, SITE_ID %in% common_site_list)

# output
write.csv(df_sens_mlm_output, "combined_source_ec_mlm_slopes.csv", row.names = F)

# reduce sites in the original list
original_site_info <- read_csv("../combined_source_site_list_info_gpp_ignore_years_cru_aridity_fixedsource.csv")
subset_site_info <- original_site_info[which(original_site_info$SITE_ID %in% common_site_list),]
write_csv(subset_site_info, "../combined_source_site_list_info_gpp_ignore_years_cru_aridity_subset2_fixedsource_mlm.csv")


# 
# #####
# # Can delete or comment the original version of univariate regression below
...
# write.csv(df_sens_sub_output, "combined_source_ec_univariate_slopes2.csv", row.names = F)
# 
# # reduce sites in the original list
# original_site_info <- read_csv("C:/Users/David Miller/Documents/dlm_files/keenan_postdoc/icos_2020_warm_winter/combined_source_site_list_info_gpp_ignore_years_cru_aridity_fixedsource.csv")
# subset_site_info <- original_site_info[which(original_site_info$SITE_ID %in% df_sens_sub_output$SITE_ID),]
# write_csv(subset_site_info, "C:/Users/David Miller/Documents/dlm_files/keenan_postdoc/icos_2020_warm_winter/combined_source_site_list_info_gpp_ignore_years_cru_aridity_subset2_fixedsource.csv")
