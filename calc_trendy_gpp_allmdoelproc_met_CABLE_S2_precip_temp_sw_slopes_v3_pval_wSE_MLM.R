# MLM version of trendy analysis!
...
latlim <- 15 # minimum latitude for analysis, 15 is beyond minimum
...
setwd("/Volumes/Helios/berkeley/TRENDY/TRENDYv6_originals_sub/")

precip <- nc_open("precip/CABLE_S2_pr.nc")
...
#image(lon,lat,pre_array[,,1], col=rev(brewer.pal(10,"RdBu")))
pre_sub <- pre_array[, latlim_inds,]
# image(lon,lat_sub,pre_sub[,,1], col=rev(brewer.pal(10,"RdBu")))
# image(lon,lat_sub,gpp_sub[,,1], col=rev(brewer.pal(10,"RdBu")))
# close connection to netcdf file and remove big array
#nc_close(precip)
rm(pre_array)
gc()
...
# AIRTEMP, K
ta <- nc_open("airtemp/CABLE_S2_tas.nc")
...
# SW, W m-2
sw <- nc_open("sw/CABLE_S2_rsds.nc")
...
#file_ind <- 15
  ...
  gpp_sub <- gpp[, latlim_inds, band_inds]
  #gpp_sub <- gpp_array[, latlim_inds, ] # don't need to subset gpp anymore by band_inds
  #pre_sub <- pre_array[, latlim_inds, band_inds] # need to load new precip specific to CABLE_S2
  yrs_sub <- yrs[band_inds]
  mos_sub <- mos[band_inds]
  ...
  detrendGPP <- function(gpp_vec, yr_vec){
    ...
    if (length(which(is.na(gpp_vec)))/length(gpp_vec) < 0.5){
      ...
      # something breaks here and mktest_result$p.value does not exist...
      # If trend is significant at 0.05, then detrend GPP and replace value, otherwise do nothing
      # test for existence of mktest_result pvalue is OK within scope of this function
      if (exists("mktest_result$p.value")){
  ...
  # multilinear version of get slopes, return as data frame instead to prevent confusion in output
  getSlopeIntInfoMLM <- function(gpp_vec, precip_vec, ta_vec, par_vec){
    
    mlm_df <- data.frame(matrix(NA, nrow = 5, ncol = 3))
    colnames(mlm_df) <- c("Precip", "Ta", "PAR")
    
    # if it's all NA or NaN or less than 2 elements not NA, don't calculate
    if (all(is.na(gpp_vec)) || all(is.na(precip_vec)) || all(is.na(ta_vec)) || all(is.na(par_vec)) ||
        length(which(!is.na(gpp_vec))) < 2 ||
        length(which(!is.na(precip_vec))) < 2 || length(which(!is.na(ta_vec))) < 2 || length(which(!is.na(par_vec))) < 2){
    # IF this is the case, don't do anything; df is already setup with NAs
    # IF this is NOT the case, proceed:
      
    } else {
      
      # mlm model
      mlm_summary <- summary(lm(gpp_vec ~ precip_vec + ta_vec + par_vec))
      
      # if there are not FOUR rows of coeffs, then it failed
      if (nrow(mlm_summary$coefficients) == 4) {
        mlm_df[1,] <- mlm_summary$coefficients[2:4,1] # slope_val
        mlm_df[2,] <- mlm_summary$coefficients[1,1] %>% rep(3) # int_val
        mlm_df[3,] <- mlm_summary$r.squared %>% rep(3) # r2_val
        mlm_df[4,] <- mlm_summary$coefficients[2:4,4] # p_val
        mlm_df[5,] <- mlm_summary$coefficients[2:4,2] # slope_se
       
        
        # DON't need this else anymore, already setup with NAs
      # } else {
      #   slope_val <- NA
      #   int_val <- NA
      #   p_val <- NA
  ...
  for (i in 1:nrow(gpp_sub_spring)){
    ...
    for (j in 1:ncol(gpp_sub_spring)){
      for (k in 1:nyrs){
        ...
        # kg C m-2 mo-1 (30 day) * 1000 g/kg * nday per month correction
        gpp_sub_spring_sum[i,j,k] <- sumna(gpp_sub_spring[i,j, yrs_inds]*1000*c(31/30, 30/30, 31/30)) # sumna fn for all na handling
        gpp_sub_summer_sum[i,j,k] <- sumna(gpp_sub_summer[i,j, yrs_inds]*1000*c(30/30, 31/30, 31/30))
        gpp_sub_fall_sum[i,j,k] <- sumna(gpp_sub_fall[i,j, yrs_inds]*1000*c(30/30, 31/30, 30/30))
      ...
      # Compute slopes MLM, gpp ~ precip + ta + par
      # Units:
      ...
      # MLM version, just do these calls once for all variables!
      gpp_MLM_slope_spring_info <- getSlopeIntInfoMLM(gpp_sub_spring_sum[i,j,], pre_sub_spring_sum[i,j,], ta_sub_spring_mean[i,j,], par_sub_spring_sum[i,j,])
      gpp_MLM_slope_summer_info <- getSlopeIntInfoMLM(gpp_sub_summer_sum[i,j,], pre_sub_summer_sum[i,j,], ta_sub_summer_mean[i,j,], par_sub_summer_sum[i,j,])
      gpp_MLM_slope_fall_info <- getSlopeIntInfoMLM(gpp_sub_fall_sum[i,j,], pre_sub_fall_sum[i,j,], ta_sub_fall_mean[i,j,], par_sub_fall_sum[i,j,])
      
      #slope
      gpp_pre_slope_spring[i,j] <- gpp_MLM_slope_spring_info$Precip[1]
      gpp_pre_slope_summer[i,j] <- gpp_MLM_slope_summer_info$Precip[1]
      gpp_pre_slope_fall[i,j] <- gpp_MLM_slope_fall_info$Precip[1]
      
      #intercept
      gpp_pre_intercept_spring[i,j] <- gpp_MLM_slope_spring_info$Precip[2]
      gpp_pre_intercept_summer[i,j] <- gpp_MLM_slope_summer_info$Precip[2]
      gpp_pre_intercept_fall[i,j] <- gpp_MLM_slope_fall_info$Precip[2]
      
      # r2
      gpp_pre_r2_spring[i,j] <- gpp_MLM_slope_spring_info$Precip[3]
      gpp_pre_r2_summer[i,j] <- gpp_MLM_slope_summer_info$Precip[3]
      gpp_pre_r2_fall[i,j] <- gpp_MLM_slope_fall_info$Precip[3]
      
      # pval
      gpp_pre_pval_spring[i,j] <- gpp_MLM_slope_spring_info$Precip[4]
      gpp_pre_pval_summer[i,j] <- gpp_MLM_slope_summer_info$Precip[4]
      gpp_pre_pval_fall[i,j] <- gpp_MLM_slope_fall_info$Precip[4]
      
      # slope se
      gpp_pre_se_spring[i,j] <- gpp_MLM_slope_spring_info$Precip[5]
      gpp_pre_se_summer[i,j] <- gpp_MLM_slope_summer_info$Precip[5]
      gpp_pre_se_fall[i,j] <- gpp_MLM_slope_fall_info$Precip[5]
      ...
      #slope
      gpp_ta_slope_spring[i,j] <- gpp_MLM_slope_spring_info$Ta[1]
      gpp_ta_slope_summer[i,j] <- gpp_MLM_slope_summer_info$Ta[1]
      gpp_ta_slope_fall[i,j] <- gpp_MLM_slope_fall_info$Ta[1]
      
      #intercept
      gpp_ta_intercept_spring[i,j] <- gpp_MLM_slope_spring_info$Ta[2]
      gpp_ta_intercept_summer[i,j] <- gpp_MLM_slope_summer_info$Ta[2]
      gpp_ta_intercept_fall[i,j] <- gpp_MLM_slope_fall_info$Ta[2]
      
      # r2
      gpp_ta_r2_spring[i,j] <- gpp_MLM_slope_spring_info$Ta[3]
      gpp_ta_r2_summer[i,j] <- gpp_MLM_slope_summer_info$Ta[3]
      gpp_ta_r2_fall[i,j] <- gpp_MLM_slope_fall_info$Ta[3]
      
      # pval
      gpp_ta_pval_spring[i,j] <- gpp_MLM_slope_spring_info$Ta[4]
      gpp_ta_pval_summer[i,j] <- gpp_MLM_slope_summer_info$Ta[4]
      gpp_ta_pval_fall[i,j] <- gpp_MLM_slope_fall_info$Ta[4]
      
      # slope se
      gpp_ta_se_spring[i,j] <- gpp_MLM_slope_spring_info$Ta[5]
      gpp_ta_se_summer[i,j] <- gpp_MLM_slope_summer_info$Ta[5]
      gpp_ta_se_fall[i,j] <- gpp_MLM_slope_fall_info$Ta[5]
      ...
      #slope
      gpp_par_slope_spring[i,j] <- gpp_MLM_slope_spring_info$PAR[1]
      gpp_par_slope_summer[i,j] <- gpp_MLM_slope_summer_info$PAR[1]
      gpp_par_slope_fall[i,j] <- gpp_MLM_slope_fall_info$PAR[1]
      
      #intercept
      gpp_par_intercept_spring[i,j] <- gpp_MLM_slope_spring_info$PAR[2]
      gpp_par_intercept_summer[i,j] <- gpp_MLM_slope_summer_info$PAR[2]
      gpp_par_intercept_fall[i,j] <- gpp_MLM_slope_fall_info$PAR[2]
      
      # r2
      gpp_par_r2_spring[i,j] <- gpp_MLM_slope_spring_info$PAR[3]
      gpp_par_r2_summer[i,j] <- gpp_MLM_slope_summer_info$PAR[3]
      gpp_par_r2_fall[i,j] <- gpp_MLM_slope_fall_info$PAR[3]
      
      # pval
      gpp_par_pval_spring[i,j] <- gpp_MLM_slope_spring_info$PAR[4]
      gpp_par_pval_summer[i,j] <- gpp_MLM_slope_summer_info$PAR[4]
      gpp_par_pval_fall[i,j] <- gpp_MLM_slope_fall_info$PAR[4]
      
      # slope se
      gpp_par_se_spring[i,j] <- gpp_MLM_slope_spring_info$PAR[5]
      gpp_par_se_summer[i,j] <- gpp_MLM_slope_summer_info$PAR[5]
      gpp_par_se_fall[i,j] <- gpp_MLM_slope_fall_info$PAR[5]
    }
  }
  ...
  # OUTPUT will be different directory, but otherwise same setup as univariate version.
  setwd("/Users/davidmiller/dlm_files/keenan_postdoc/trendy/gpp_allmodel_slope_error_mlm/")
  ...
  write_csv(df_spring, paste(prefix, "gppproc_MLM_met_CABLE_S2_precip_ta_par_df_spring_slope_se.csv", sep = ""))
  ...
  write_csv(df_summer, paste(prefix, "gppproc_MLM_met_CABLE_S2_precip_ta_par_df_summer_slope_se.csv", sep = ""))
  ...
  write_csv(df_fall, paste(prefix, "gppproc_MLM_met_CABLE_S2_precip_ta_par_df_fall_slope_se.csv", sep = ""))
}
