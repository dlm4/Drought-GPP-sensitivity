...
# NEW # read in CRU aridity
setwd("/Users/davidmiller/dlm_files/keenan_postdoc/trendy/aridity/")
site_cru_aridity <- read.csv("site_cru_aridity_1981_2010.csv") # substitute this in for the TBM WI
...
# substitute the TBM WI from CRU 4.5 here
tbm_inds <- which(model_ec_site_slopes_tc_spring$ModelName %in% c("VEGAS", "TRENDY_model_mean", "LPJ-GUESS", "LPJ", "ORCHIDEE", 
                                                      "ORCHIDEE-MICT", "CABLE", "JSBACH", "JULES", "VISIT", 
                                                      "CLASS-CTEM", "CLM", "ISAM", "DLEM", "LPX"))
for (i in tbm_inds){
  selected_row <- model_ec_site_slopes_tc_spring[i,] 
  cru_row <- which(site_cru_aridity$SITE_ID == selected_row$SITE_ID)
  cru_aridity <- site_cru_aridity$Aridity_P_PET[cru_row] # cru aridity
  model_ec_site_slopes_tc_spring$TC_aridity[i] <- cru_aridity
}


#
model_ec_site_slopes_tc_spring$threshold <- "Energy-limited (Wet)"
model_ec_site_slopes_tc_spring$threshold[which(model_ec_site_slopes_tc_spring$TC_aridity < arid_threshold)] <- "Water-limited (Dry)" ## will need to adjust this line to be based on CRU ##
...
setwd("/Users/davidmiller/dlm_files/keenan_postdoc/terraclimate/processed_data/figures/updated_figs")
# 0.65
ggsave("gpp_model_wet_dry_compare_cru_TBM.eps", wet_dry_compare, width = 120, height = 120, units = "mm", dpi = 300)
#ggsave("gpp_model_wet_dry_compare_clean_cru_TBM.eps", wet_dry_compare, width = 120, height = 95, units = "mm", dpi = 300) # version without text
